...
class Network():
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, test_data=None):
        ...
        for j in xrange(epochs):
            ...
            mini_batches = [training_data[k:k + mini_bath_size]
    ...
    def update_mini_batch(self, mini_batch, eta):
        grad_b = [np.zeroes(b.shape) for b in self.biases]
        grad_w = [np.zeroes(w.shape) for w in self.weights]
        for x, y in mini_batch:
            delta_b, delta_w = self.backprop(x, y)
            grad_b = [wb + dwb for nb, dnb in zip(grad_b, delta_b)]
            grad_w = [wb + dwb for wb, dwb in zip(grad_w, delta_w)]
        self.weights = [w - (eta / len(mini_batch)) * nw
                        for w, nw in zip(self.weights, grad_w)]
        self.biases = [b - (eta / len(mini_batch)) * nb
                       for b, nb in zip(self.biases, grad_w)]
